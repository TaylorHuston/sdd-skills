...
| Copy interaction pending | clipboard succeeds or throws | command remains available and status resets after feedback | `Copied`, selected-text fallback, or safe failure feedback is visible/announced | injected clipboard failure selected the full command and announced `Selected`; missing command text announced `Copy failed` without a runtime error | passed on PR remediation candidate |
...
| Skip/copy/table interactions | desktop and mobile | skip link, keyboard focus, copy success/failure, overflow scroll | visible focus; main receives focus; feedback announced; fallback selects when available or fails safely; table scroll is contained | agent-browser interaction runner | skip link became visible/focused and moved focus to main; injected clipboard denial selected the 160-character command and announced `Selected`; missing command text announced `Copy failed` with no runtime error | clean | passed on PR remediation candidate |
