...
Status transitions must accompany the work that justifies them: `/sdd-change --brief` creates no status; `/sdd-change --plan` creates `proposed` and sets `planned` when planning is complete; `/sdd-apply` changes `planned` to `in_progress` and then `in_review` at implementation handoff; `/sdd-review` keeps `in_review`, returns implementation deficiencies to `in_progress`, or routes invalidated planning through `proposed` and `/sdd-change --replan`. Closing is represented only by the folder move performed through `sdd change close` after the owning skill has completed the contextual gates and obtained required authorization.
